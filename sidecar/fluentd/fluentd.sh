...
curl -XPUT "http://localhost:9200/job_log/_mapping" \
    ...
    --data "@reana-demo-logs/sidecar/fluentd/log_mapping.json"
curl -XPUT "http://localhost:9200/workflow_log/_mapping" \
    -H 'Content-Type: application/json' \
    --data "@reana-demo-logs/sidecar/fluentd/log_mapping.json"

kubectl apply -f reana-demo-logs/sidecar/fluentd/fluentd-config.yaml
kubectl apply -f reana-demo-logs/sidecar/fluentd/fluentd-config-workflow.yaml
